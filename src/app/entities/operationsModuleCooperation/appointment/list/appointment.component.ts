...
export class AppointmentComponent implements OnInit {
  private static readonly NOT_SORTABLE_FIELDS_AFTER_SEARCH = [
    ...
    'clientID',
    'clientName',
    'clientContactNumber1',
    'clientContactNumber2',
    'clientAddress',
    'clientCity',
    'remarks',
    'appointmentType',
  ...
  search(query: string): void {
    ...
    if (query && predicate && AppointmentComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      const { predicate } = this.sortState();
      if (predicate && AppointmentComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
        this.sortState.set({});
      }
    }
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.appointmentService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
