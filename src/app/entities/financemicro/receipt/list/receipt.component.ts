...
export class ReceiptComponent implements OnInit {
  ...
  trackId = (item: IReceipt): number => this.receiptService.getReceiptIdentifier(item);
  ...
  search(query: string): void {
    ...
    if (query && predicate && ReceiptComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      const { predicate } = this.sortState();
      if (predicate && ReceiptComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
        this.sortState.set({});
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.receipts.set(dataFromBody);
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.receiptService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
