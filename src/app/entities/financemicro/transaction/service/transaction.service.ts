...
type RestOf<T extends ITransaction | NewTransaction> = Omit<T, 'date' | 'lmd'> & {
  date?: string | null;
  lmd?: string | null;
};
...
export class TransactionService {
  ...
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/transactions', 'financemicro');
  protected resourceSearchUrl = this.applicationConfigService.getEndpointFor('api/transactions/_search', 'financemicro');
  ...
  protected convertDateFromClient<T extends ITransaction | NewTransaction | PartialUpdateTransaction>(transaction: T): RestOf<T> {
    return {
      ...
      lmd: transaction.lmd?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restTransaction: RestTransaction): ITransaction {
    return {
      ...
      lmd: restTransaction.lmd ? dayjs(restTransaction.lmd) : undefined,
    };
  }
