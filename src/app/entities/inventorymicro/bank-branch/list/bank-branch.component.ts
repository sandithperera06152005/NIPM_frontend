...
import { BankBranchService, EntityArrayResponseType } from '../service/bank-branch.service';
...
export class BankBranchComponent implements OnInit {
  ...
  search(query: string): void {
    ...
    if (query && predicate && BankBranchComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      const { predicate } = this.sortState();
      if (predicate && BankBranchComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
        this.sortState.set({});
      }
    }
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.bankBranchService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
