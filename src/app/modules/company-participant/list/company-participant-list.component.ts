...
// Application Imports
import { ICompanyParticipant } from '../company-participant.model';
import { CompanyParticipantService } from '../service/company-participant.service';
import { CompanyParticipantFormComponent } from '../form/company-participant-form.component';
...
@Component({
  selector: 'app-company-participant-list',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
    ...
    MatDatepickerModule,
    MatNativeDateModule,
    CompanyParticipantFormComponent,
  ],
  templateUrl: './company-participant-list.component.html',
})
export class CompanyParticipantListComponent implements AfterViewInit, OnInit {
  // --- Injected Services ---
  ...
  private readonly dialogRef = inject(MatDialogRef<CompanyParticipantListComponent>, { optional: true });
  ...
  selectedCompanyParticipant: ICompanyParticipant | null = null;
  ...
  displayedColumns: string[] = ['id', 'joinedAt', 'actions'];
  dataSource = new MatTableDataSource<ICompanyParticipant>();
  ...
  loadData(): void {
    ...
    this.companyParticipantService.query(req).pipe(
      tap(res => {
  ...
  openFormDrawer(id?: number): void {
    if (id) {
      ...
      this.companyParticipantService.find(id).subscribe(response => {
        if (response.body) {
          this.selectedCompanyParticipant = response.body;
          this.formDrawer.open();
        }
      });
    } else {
      this.drawerMode = 'new';
      this.selectedCompanyParticipant = null;
      this.formDrawer.open();
    }
  }
  ...
  delete(id: number): void {
    const confirmation = this.fuseConfirmationService.open({
      title: 'Delete CompanyParticipant',
    ...
    confirmation.afterClosed().subscribe(result => {
      if (result === 'confirmed') {
        this.companyParticipantService.delete(id).subscribe(() => {
          this.refreshTrigger.next();
          this.loadData();
        });
      }
