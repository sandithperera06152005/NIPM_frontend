...
import { MembershipTypeFormComponent } from '../form/membership-type-form.component';
...
export class MembershipTypeListComponent implements AfterViewInit, OnInit {
  // --- Injected Services ---
  ...
  private readonly dialogRef = inject(MatDialogRef<MembershipTypeListComponent>, { optional: true });
  ...
  selectedMembershipType: IMembershipType | null = null;
  ...
  displayedColumns: string[] = ['id', 'name', 'fee', 'validityMonths',  'actions'];
  dataSource = new MatTableDataSource<IMembershipType>();
  ...
  loadData(): void {
    ...
    this.membershipTypeService.query(req).pipe(
      tap(res => {
  ...
  openFormDrawer(id?: number): void {
    if (id) {
      ...
      this.membershipTypeService.find(id).subscribe(response => {
        if (response.body) {
          this.selectedMembershipType = response.body;
