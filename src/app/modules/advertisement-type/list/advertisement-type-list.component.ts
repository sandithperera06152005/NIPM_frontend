...
// Application Imports
import { IAdvertisementType } from '../advertisement-type.model';
import { AdvertisementTypeService } from '../service/advertisement-type.service';
import { AdvertisementTypeFormComponent } from '../form/advertisement-type-form.component';
...
@Component({
  selector: 'app-advertisement-type-list',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
    ...
    MatDatepickerModule,
    MatNativeDateModule,
    AdvertisementTypeFormComponent,
  ],
  templateUrl: './advertisement-type-list.component.html',
})
export class AdvertisementTypeListComponent implements AfterViewInit, OnInit {
  // --- Injected Services ---
  ...
  private readonly dialogRef = inject(MatDialogRef<AdvertisementTypeListComponent>, { optional: true });
  ...
  selectedAdvertisementType: IAdvertisementType | null = null;
  ...
  dataSource = new MatTableDataSource<IAdvertisementType>();
  ...
  loadData(): void {
    ...
    this.advertisementTypeService.query(req).pipe(
      tap(res => {
  ...
  openFormDrawer(id?: number): void {
    if (id) {
      ...
      this.advertisementTypeService.find(id).subscribe(response => {
        if (response.body) {
          this.selectedAdvertisementType = response.body;
          this.formDrawer.open();
        }
      });
    } else {
      this.drawerMode = 'new';
      this.selectedAdvertisementType = null;
      this.formDrawer.open();
    }
  }
  ...
  delete(id: number): void {
    const confirmation = this.fuseConfirmationService.open({
      title: 'Delete AdvertisementType',
    ...
    confirmation.afterClosed().subscribe(result => {
      if (result === 'confirmed') {
        this.advertisementTypeService.delete(id).subscribe(() => {
          this.refreshTrigger.next();
          this.loadData();
        });
      }
