...
export class AccountstatementComponent {
  ...
  fetchData() {
    ...
    const accountCode = this.grnForm.value.accountCode;

    const params: any = { size: 10000000 };

    if (accountCode) params['accountCode.equals'] = accountCode;
  ...
  this.flatRelatedData = this.transactiondata.map((item, index) => ({
    ...item,
    groupIndex: index + 1,
    itemName: item.accountCode || 'N/A',
    relid: item.relid ?? '—',
    accountCode: item.accountCode || 'N/A',
    date: item.date,
    debit: item.debit || 0,
    credit: item.credit || 0,
    refDoc: item.refDoc || 'N/A',
  ...
  printStatement() {
    if (this.flatRelatedData.length > 0) {
      setTimeout(() => {
        window.print();
      }, 500);
    }
  ...
  paginatedRows: any[][] = [];
  fetchaccdet() {
  const supplierCode = this.grnForm.value.accountCode;
  if (!supplierCode) return;

  this.AccountsService.query({
    'code.equals': supplierCode,
    size: 1
  }).subscribe({
    next: res => {
      this.res = res.body?.[0] ?? {};
  ...
  check() {
    ...
    this.items.forEach(item => {
      this.AccountsService.query({ 'code.equals': item.itemCode, size: 1000, page: 0 }).subscribe({
        next: (response) => {
  ...
  searchvendor(): void {
  const searchValue = this.grnForm.value.accountCode;
  if (!searchValue) return;

  this.suplierprof.query({
    'name.contains': searchValue,
    size: 1000
  }).subscribe({
    next: res => {
      this.searchResultsvendor = res.body || [];
    }
  });
}
...
onItemSelectedvendor(code: string): void {
  this.grnForm.patchValue({
    accountCode: code
  });

  console.log('Selected accountCode:', code);
}
  ...
  ngOnInit(): void {
    ...
    this.updatePagedItems();
    this.grnForm.get('accountCode')?.valueChanges
