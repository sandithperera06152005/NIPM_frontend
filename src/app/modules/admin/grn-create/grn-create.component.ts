...
if (!this.suid) {
 ...
 // }
   ...
   this.createGRNAccountEntry();

 }


 private createGRNAccountEntry(): void {
  const supplierName = this.grnForm.value.supplierName || 'GRN Supplier';
  const debitAmount = this.items.reduce((sum, item) => sum + (item.purcost || 0), 0);

  if (debitAmount <= 0) {
    console.warn('No debit amount to process for GRN account entry');
    return;
  }

  // Check if GRN supplier account already exists
  const params = {
  ...
  this.AccountsService.query(params).subscribe({
    next: (response) => {
      ...
      if (existingAccounts.length > 0) {
        // Update existing account - ADD to DEBIT column
        this.updateExistingGRNAccount(existingAccounts[0], debitAmount, supplierName);
      } else {
        // Create new account
        this.createNewGRNAccount(supplierName, debitAmount);
      }
        this.updateInventoryAccount();

    },
    error: (error) => {
      console.error('Error checking existing GRN accounts:', error);
      this.createNewGRNAccount(supplierName, debitAmount);
      this.updateInventoryAccount();
    }
  });
...
private updateExistingGRNAccount(existingAccount: any, debitAmount: number, supplierName: string): void {
  ...
  this.AccountsService.update(updatedAccount).subscribe({
    next: (response) => {
      console.log(`GRN account for ${supplierName} updated successfully. Debit amount:`, newDebitAmount);
    },
    error: (error) => {
      console.error(`Error updating GRN account for ${supplierName}:`, error);
    }
  });
}

private createNewGRNAccount(supplierName: string, debitAmount: number): void {
  const randomNum = Math.floor(1000 + Math.random() * 9000);
  const accountCode = `GRN${randomNum}`;
  ...
  const accountData: any = {
    ...
    path: `Liability/Suppliers/${supplierName}` // Adjust path as needed
  };

  this.AccountsService.create(accountData).subscribe({
    next: (response) => {
      console.log(`New GRN account created for ${supplierName}:`, response);
    },
    error: (error) => {
      console.error(`Error creating GRN account for ${supplierName}:`, error);
    }
  });
}

private updateInventoryAccount(): void {
  const creditAmount = this.items.reduce((sum, item) => sum + (item.purcost || 0), 0);

  if (creditAmount <= 0) {
    console.warn('No credit amount to process for inventory account');
    return;
  }

  // Check if inventory account already exists
  const params = {
  ...
  this.AccountsService.query(params).subscribe({
    next: (response) => {
      const existingInventoryAccounts = response.body || [];
      
      if (existingInventoryAccounts.length > 0) {
        // Update existing inventory account - ADD to CREDIT column
        this.updateExistingInventoryAccount(existingInventoryAccounts[0], creditAmount);
      } else {
    ...
    error: (error) => {
      console.error('Error checking existing inventory accounts:', error);
      this.createNewInventoryAccount(creditAmount);
    }
  });
}

private updateExistingInventoryAccount(existingAccount: any, creditAmount: number): void {
...
private createNewInventoryAccount(creditAmount: number): void {
  const randomNum = Math.floor(1000 + Math.random() * 9000);
  ...
  const inventoryAccountData: any = {
    ...
    debitAmount: null, 
    path: 'Asset/Current Assets/Inventory' // Adjust path as needed
  };
