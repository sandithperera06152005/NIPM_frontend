...
export class JobCardsComponent implements OnInit, AfterViewInit, OnDestroy {
  private _fuseConfirmationService = inject(FuseConfirmationService);
  private jobCardService = inject(JobCardService);
  private router = inject(Router);
  private dialog = inject(MatDialog);

  dataSource: MatTableDataSource<IJobCard> = new MatTableDataSource<IJobCard>(
    []
  );
  ...
  openJobCardEditDialog(jobCard: IJobCard): void {
    // Implement the logic to open the edit dialog
    console.log("Open edit dialog", jobCard);
  }
  ...
  deleteJobCard(jobCard: IJobCard): void {
    ...
    confirmation.afterClosed().subscribe((result) => {
      if (result === "confirmed") {
        this.jobCardService.delete(jobCard.id).subscribe(() => {
          this.loadJobCards();
        });
      }
    });
