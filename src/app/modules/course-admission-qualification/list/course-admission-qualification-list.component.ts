...
// Application Imports
import { ICourseAdmissionQualification } from '../course-admission-qualification.model';
import { CourseAdmissionQualificationService } from '../service/course-admission-qualification.service';
import { CourseAdmissionQualificationFormComponent } from '../form/course-admission-qualification-form.component';
...
@Component({
  selector: 'app-course-admission-qualification-list',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
    ...
    MatDatepickerModule,
    MatNativeDateModule,
    CourseAdmissionQualificationFormComponent,
  ],
  templateUrl: './course-admission-qualification-list.component.html',
})
export class CourseAdmissionQualificationListComponent implements AfterViewInit, OnInit {
  // --- Injected Services ---
  ...
  private readonly dialogRef = inject(MatDialogRef<CourseAdmissionQualificationListComponent>, { optional: true });
  ...
  selectedCourseAdmissionQualification: ICourseAdmissionQualification | null = null;
  ...
  displayedColumns: string[] = ['id', 'subjectName', 'grade', 'year', 'level', 'actions'];
  dataSource = new MatTableDataSource<ICourseAdmissionQualification>();
  ...
  loadData(): void {
    ...
    this.courseAdmissionQualificationService.query(req).pipe(
      tap(res => {
  ...
  openFormDrawer(id?: number): void {
    if (id) {
      ...
      this.courseAdmissionQualificationService.find(id).subscribe(response => {
        if (response.body) {
          this.selectedCourseAdmissionQualification = response.body;
          this.formDrawer.open();
        }
      });
    } else {
      this.drawerMode = 'new';
      this.selectedCourseAdmissionQualification = null;
      this.formDrawer.open();
    }
  }
  ...
  delete(id: number): void {
    const confirmation = this.fuseConfirmationService.open({
      title: 'Delete CourseAdmissionQualification',
    ...
    confirmation.afterClosed().subscribe(result => {
      if (result === 'confirmed') {
        this.courseAdmissionQualificationService.delete(id).subscribe(() => {
          this.refreshTrigger.next();
          this.loadData();
        });
      }
