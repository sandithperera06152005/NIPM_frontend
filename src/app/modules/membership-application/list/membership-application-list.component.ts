...
// Application Imports
import { IMembershipApplication } from '../membership-application.model';
import { MembershipApplicationService } from '../service/membership-application.service';
import { MembershipApplicationFormComponent } from '../form/membership-application-form.component';
...
@Component({
  selector: 'app-membership-application-list',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
  ...
  templateUrl: './membership-application-list.component.html',
})
export class MembershipApplicationListComponent implements AfterViewInit, OnInit {
  // --- Injected Services ---
  ...
  private readonly dialogRef = inject(MatDialogRef<MembershipApplicationListComponent>, { optional: true });
  ...
  loadData(): void {
    ...
    this.membershipApplicationService.query(req).pipe(
      tap(res => {
  ...
  openFormDrawer(id?: number): void {
    if (id) {
      ...
      this.membershipApplicationService.find(id).subscribe(response => {
        if (response.body) {
          this.selectedMembershipApplication = response.body;
          this.formDrawer.open();
        }
      });
    } else {
      this.drawerMode = 'new';
      this.selectedMembershipApplication = null;
      this.formDrawer.open();
    }
  }
  ...
  delete(id: number): void {
    const confirmation = this.fuseConfirmationService.open({
      title: 'Delete MembershipApplication',
    ...
    confirmation.afterClosed().subscribe(result => {
      if (result === 'confirmed') {
        this.membershipApplicationService.delete(id).subscribe(() => {
          this.refreshTrigger.next();
          this.loadData();
        });
      }
