...
@Component({
  selector: 'app-membership-admission-list',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
...
export class MembershipAdmissionListComponent implements AfterViewInit, OnInit {
  ...
  openFormDrawer(id?: number): void {
    if (id) {
      ...
      this.membershipAdmissionService.find(id).subscribe(response => {
        if (response.body) {
          this.selectedMembershipAdmission = response.body;
          this.formDrawer.open();
        }
      });
    } else {
      this.drawerMode = 'new';
      this.selectedMembershipAdmission = null;
      this.formDrawer.open();
    }
  }
  ...
  delete(id: number): void {
    const confirmation = this.fuseConfirmationService.open({
      title: 'Delete MembershipAdmission',
