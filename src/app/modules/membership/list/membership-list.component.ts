...
export class MembershipListComponent implements AfterViewInit, OnInit {
  ...
  private readonly dialogRef = inject(MatDialogRef<MembershipListComponent>, { optional: true });
  ...
  displayedColumns: string[] = ['id', 'membershipNumber', 'startDate', 'endDate', 'status',  'actions'];
  dataSource = new MatTableDataSource<IMembership>();
  ...
  loadData(): void {
    ...
    this.membershipService.query(req).pipe(
      tap(res => {
  ...
  openFormDrawer(id?: number): void {
    if (id) {
      ...
      this.membershipService.find(id).subscribe(response => {
        if (response.body) {
          this.selectedMembership = response.body;
