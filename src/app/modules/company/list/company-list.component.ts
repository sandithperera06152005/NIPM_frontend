...
export class CompanyListComponent implements AfterViewInit, OnInit {
  ...
  private readonly dialogRef = inject(MatDialogRef<CompanyListComponent>, { optional: true });
  ...
  displayedColumns: string[] = ['id', 'companyName', 'registrationNo', 'contactEmail', 'contactPhone', 'address', 'status', 'createdAt', 'actions'];
  ...
  loadData(): void {
    ...
    this.companyService.query(req).pipe(
      tap(res => {
  ...
  openFormDrawer(id?: number): void {
    if (id) {
      ...
      this.companyService.find(id).subscribe(response => {
        if (response.body) {
