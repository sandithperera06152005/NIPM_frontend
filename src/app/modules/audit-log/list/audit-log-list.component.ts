...
import { AuditLogFormComponent } from '../form/audit-log-form.component';
...
export class AuditLogListComponent implements AfterViewInit, OnInit {
  // --- Injected Services ---
  private readonly auditLogService = inject(AuditLogService);
  private readonly fuseConfirmationService = inject(FuseConfirmationService);
  private readonly route = inject(ActivatedRoute);
  private readonly dialog = inject(MatDialog);
  private readonly dialogRef = inject(MatDialogRef<AuditLogListComponent>, { optional: true });
  ...
  displayedColumns: string[] = ['id', 'action', 'entityName', 'entityId', 'performedAt', 'ipAddress', 'actions'];
  ...
  loadData(): void {
    ...
    this.auditLogService.query(req).pipe(
      tap(res => {
  ...
  openFormDrawer(id?: number): void {
    if (id) {
      ...
      this.auditLogService.find(id).subscribe(response => {
        if (response.body) {
          this.selectedAuditLog = response.body;
