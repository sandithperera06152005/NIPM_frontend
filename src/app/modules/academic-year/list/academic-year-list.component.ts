...
import { AcademicYearFormComponent } from '../form/academic-year-form.component';
...
export class AcademicYearListComponent implements AfterViewInit, OnInit {
  // --- Injected Services ---
  private readonly academicYearService = inject(AcademicYearService);
  private readonly fuseConfirmationService = inject(FuseConfirmationService);
  private readonly route = inject(ActivatedRoute);
  private readonly dialog = inject(MatDialog);
  private readonly dialogRef = inject(MatDialogRef<AcademicYearListComponent>, { optional: true });
  ...
  selectedAcademicYear: IAcademicYear | null = null;
  ...
  displayedColumns: string[] = ['id', 'yearLabel', 'startDate', 'endDate',  'actions'];
  dataSource = new MatTableDataSource<IAcademicYear>();
  ...
  loadData(): void {
    ...
    this.academicYearService.query(req).pipe(
      tap(res => {
  ...
  openFormDrawer(id?: number): void {
    if (id) {
      ...
      this.academicYearService.find(id).subscribe(response => {
        if (response.body) {
          this.selectedAcademicYear = response.body;
