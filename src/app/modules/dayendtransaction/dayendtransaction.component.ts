...
@Component({
 ...
 standalone: true,
  ...
  templateUrl: './dayendtransaction.component.html',
  styleUrl: './dayendtransaction.component.scss'
})
export class DayendtransactionComponent {
...
fetchData() {
  ...
  // Extract grndaterange from URL
  const grndaterange = this.route.snapshot.queryParamMap.get('grndaterange');

  let startDate: Date | null = null;
  let endDate: Date | null = null;

  if (grndaterange) {
    const [startRaw, endRaw] = grndaterange.split('|');
    startDate = new Date(startRaw);
    endDate = new Date(endRaw);
    params['date.greaterThanOrEqual'] = startDate.toISOString();
    params['date.lessThanOrEqual'] = endDate.toISOString();
  }

  const supplierName = this.grnForm.value.supplierName;
  if (supplierName) {
    params['accountCode.equals'] = supplierName;
  }
  ...
  this.transacation.query(params).subscribe({
    next: (response) => {
      ...
      // Group by relid (or some other field if needed), but no second API call
      this.groupedRelatedData = [];
      ...
      this.transactiondata.forEach(item => {
        ...
        if (tempGroup.length === 2) {
          this.groupedRelatedData.push([...tempGroup]);
          tempGroup = [];
          groupIndex++;
        }
      });
      ...
      this.flatRelatedData = this.transactiondata.map((item, i) => ({
        ...item,
        groupIndex: Math.floor(i / 2) + 1,
        itemName: item.itemName || item.name || 'N/A'
      }));

      const openingBalance = Number(this.transactiondata[0]?.subId || 0);
      let totalCredit = 0;
      let totalDebit = 0;
...
onItemSelected(selectedItem: any): void {
  ...
  if (this.suid) {
    ...
    this.grnservice.query({ 'id.equals': this.suid }).subscribe({
     ...
     // close drawer if suid exists (editing existing GRN)
  } else {
     // open drawer if no suid (creating new GRN)
  }
});
