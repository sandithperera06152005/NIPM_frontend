...
// Application Imports
import { ICourseApplication } from '../course-application.model';
import { CourseApplicationService } from '../service/course-application.service';
import { CourseApplicationFormComponent } from '../form/course-application-form.component';
...
@Component({
  selector: 'app-course-application-list',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
    ...
    MatDatepickerModule,
    MatNativeDateModule,
    CourseApplicationFormComponent,
  ],
  templateUrl: './course-application-list.component.html',
})
export class CourseApplicationListComponent implements AfterViewInit, OnInit {
  // --- Injected Services ---
  ...
  private readonly dialogRef = inject(MatDialogRef<CourseApplicationListComponent>, { optional: true });
  ...
  selectedCourseApplication: ICourseApplication | null = null;
  ...
  displayedColumns: string[] = ['id', 'applicationDate', 'status', 'remarks', 'actions'];
  dataSource = new MatTableDataSource<ICourseApplication>();
  ...
  loadData(): void {
    ...
    this.courseApplicationService.query(req).pipe(
      tap(res => {
  ...
  openFormDrawer(id?: number): void {
    if (id) {
      ...
      this.courseApplicationService.find(id).subscribe(response => {
        if (response.body) {
          this.selectedCourseApplication = response.body;
          this.formDrawer.open();
        }
      });
    } else {
      this.drawerMode = 'new';
      this.selectedCourseApplication = null;
      this.formDrawer.open();
    }
  }
  ...
  delete(id: number): void {
    const confirmation = this.fuseConfirmationService.open({
      title: 'Delete CourseApplication',
    ...
    confirmation.afterClosed().subscribe(result => {
      if (result === 'confirmed') {
        this.courseApplicationService.delete(id).subscribe(() => {
          this.refreshTrigger.next();
          this.loadData();
        });
      }
